...
RepoFix-Harness 是一个面向 Python 仓库 Bug 修复的 Coding Agent Harness。真实 LLM 自主查看、搜索和修改代码；Harness 负责有界上下文、阶段工具权限、预算、checkpoint、Docker 测试、回滚和独立评测。项目从三条重复失败定位阶段策略缺陷，在同预算 ItsDangerous 门禁中把 0/3 改进为 3/3；随后又用三个真实上游 family、九次 Flash 复验确认 9/9 且没有额外导航。
...
V4.2 可用于回答“修一个案例会不会过拟合”：用 last、numeric_range 和 Click 三类既有真实上游合同各重复三次，得到 9/9 修复与范围命中、0 retries；所有轨迹都没触发新阶段，因为它们在五次导航前已经补丁。新策略因此通过了正向触发和负向不触发两类门禁。

V4.3 可用于回答“如何证明策略确实触发或没有触发”：报告不再只保存原始 `context_snapshots`，而是从 task 明细重算 phase 次数、真实 phase transition 和 target-read grace 激活次数。冻结 V4.2 数据离线回算为 43 snapshots、0 次激活；schema v2 若被手工改成其他数字会在发布前拒绝。这把可观测性从“有 trace”推进到“可聚合、可校验的策略证据”。
...
V4.6 可用于回答“新报告功能只在单测里工作吗”：用 DeepSeek Flash/non-thinking 对三个 Docker fixture 做原生 schema-v3 门禁，3/3 修复且范围命中，12 requests / 22,205 tokens / 0 retries。最终 report.md 直接展示 overall、variant 和三个 case 的 snapshot 计数，validator 再从 12 个 task snapshots 重算全部聚合，因此真实 API 路径和报告路径同时闭环。
...
- **下一步是什么？** V4.6 已原生验证 schema-v3 单模型多 case 报告；下一步可以用相同三案例做一次严格 Flash/Pro 或 non-thinking/thinking 对照，原生验证 phase delta，但应先权衡额外 API 成本和三次小样本价值。
